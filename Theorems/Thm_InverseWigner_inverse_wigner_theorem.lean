...
theorem inverse_wigner_theorem
    ...
    (r : ℕ) (p : ℕ → ℕ) :
    Filter.Tendsto
      (fun N : ℕ => ∫ A, ∏ i ∈ Finset.range r, diagRe (A * B N * Aᴴ) i ^ p i ∂(μ N))
      Filter.atTop
      (nhds (∏ i ∈ Finset.range r,
        ∫ x, x ^ p i ∂ProbabilityTheory.gaussianReal 0 (γ 2).toNNReal)) := by
  sorry
